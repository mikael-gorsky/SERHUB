-- SERHUB Reset and Migration
-- Drops all existing SERHUB objects and recreates them
...
-- Drop all functions with CASCADE (this will drop dependent triggers too)
drop function if exists serhub_update_updated_at() cascade;
drop function if exists serhub_log_audit() cascade;
drop function if exists serhub_get_section_progress(uuid) cascade;
drop function if exists serhub_get_section_status(uuid) cascade;
drop function if exists serhub_get_user_full_name(uuid) cascade;
drop function if exists serhub_handle_new_user() cascade;
drop function if exists serhub_get_overall_progress() cascade;
drop function if exists serhub_get_user_task_stats(uuid) cascade;
drop function if exists serhub_get_user_role() cascade;
drop function if exists serhub_is_admin() cascade;
drop function if exists serhub_is_coordinator_or_admin() cascade;

-- Drop tables with CASCADE
drop table if exists serhub_notification_preferences cascade;
drop table if exists serhub_audit_log cascade;
drop table if exists serhub_meeting_participants cascade;
drop table if exists serhub_meetings cascade;
drop table if exists serhub_task_dependencies cascade;
drop table if exists serhub_task_collaborators cascade;
drop table if exists serhub_tasks cascade;
drop table if exists serhub_sections cascade;
drop table if exists serhub_profiles cascade;

-- Now drop the function that depends on serhub_tasks type (after table is dropped)
drop function if exists serhub_get_task_status_label cascade;
...
-- PROFILES POLICIES
create policy "profiles_select_all" on serhub_profiles for select to authenticated using (true);
create policy "profiles_update_own" on serhub_profiles for update to authenticated using (id = auth.uid()) with check (id = auth.uid());
create policy "profiles_update_admin" on serhub_profiles for update to authenticated using (serhub_is_admin()) with check (serhub_is_admin());
create policy "profiles_insert_own" on serhub_profiles for insert to authenticated with check (id = auth.uid());

-- SECTIONS POLICIES
create policy "sections_select_all" on serhub_sections for select to authenticated using (true);
create policy "sections_insert_admin" on serhub_sections for insert to authenticated with check (serhub_is_admin());
create policy "sections_update_admin" on serhub_sections for update to authenticated using (serhub_is_admin()) with check (serhub_is_admin());
create policy "sections_delete_admin" on serhub_sections for delete to authenticated using (serhub_is_admin());

-- TASKS POLICIES
create policy "tasks_select_all" on serhub_tasks for select to authenticated using (true);
create policy "tasks_insert_coordinator" on serhub_tasks for insert to authenticated with check (serhub_is_coordinator_or_admin());
create policy "tasks_update_involved" on serhub_tasks for update to authenticated
  using (owner_id = auth.uid() or reviewer_id = auth.uid() or approver_id = auth.uid()
    or exists (select 1 from serhub_task_collaborators where task_id = serhub_tasks.id and user_id = auth.uid())
    or serhub_is_coordinator_or_admin())
  with check (owner_id = auth.uid() or reviewer_id = auth.uid() or approver_id = auth.uid()
    or exists (select 1 from serhub_task_collaborators where task_id = serhub_tasks.id and user_id = auth.uid())
    or serhub_is_coordinator_or_admin());
create policy "tasks_delete_coordinator" on serhub_tasks for delete to authenticated using (serhub_is_coordinator_or_admin());

-- TASK COLLABORATORS POLICIES
create policy "task_collaborators_select_all" on serhub_task_collaborators for select to authenticated using (true);
create policy "task_collaborators_insert" on serhub_task_collaborators for insert to authenticated
  with check (exists (select 1 from serhub_tasks where id = task_id and owner_id = auth.uid()) or serhub_is_coordinator_or_admin());
create policy "task_collaborators_delete" on serhub_task_collaborators for delete to authenticated
  using (exists (select 1 from serhub_tasks where id = task_id and owner_id = auth.uid()) or serhub_is_coordinator_or_admin());

-- TASK DEPENDENCIES POLICIES
create policy "task_dependencies_select_all" on serhub_task_dependencies for select to authenticated using (true);
create policy "task_dependencies_insert" on serhub_task_dependencies for insert to authenticated
  with check (exists (select 1 from serhub_tasks where id = task_id and owner_id = auth.uid()) or serhub_is_coordinator_or_admin());
create policy "task_dependencies_delete" on serhub_task_dependencies for delete to authenticated
  using (exists (select 1 from serhub_tasks where id = task_id and owner_id = auth.uid()) or serhub_is_coordinator_or_admin());
...
create policy "meetings_delete_coordinator" on serhub_meetings for delete to authenticated using (serhub_is_coordinator_or_admin());

-- MEETING PARTICIPANTS POLICIES
create policy "meeting_participants_select_all" on serhub_meeting_participants for select to authenticated using (true);
create policy "meeting_participants_insert" on serhub_meeting_participants for insert to authenticated
  with check (exists (select 1 from serhub_meetings where id = meeting_id and created_by = auth.uid()) or serhub_is_coordinator_or_admin());
create policy "meeting_participants_delete" on serhub_meeting_participants for delete to authenticated
  using (exists (select 1 from serhub_meetings where id = meeting_id and created_by = auth.uid()) or serhub_is_coordinator_or_admin());

-- AUDIT LOG POLICIES
create policy "audit_log_select_admin" on serhub_audit_log for select to authenticated using (serhub_is_admin());

-- NOTIFICATION PREFERENCES POLICIES
create policy "notification_prefs_select_own" on serhub_notification_preferences for select to authenticated using (user_id = auth.uid());
create policy "notification_prefs_insert_own" on serhub_notification_preferences for insert to authenticated with check (user_id = auth.uid());
create policy "notification_prefs_update_own" on serhub_notification_preferences for update to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());
...
declare
  ...
  child_count integer;
begin
  select count(*), coalesce(avg(status), 0) into task_count, direct_task_avg from serhub_tasks where section_id = section_uuid;
  select count(*), coalesce(avg(serhub_get_section_progress(id)), 0) into child_count, child_section_avg from serhub_sections where parent_id = section_uuid;
  if task_count + child_count = 0 then return 0; end if;
...
begin
  select exists (select 1 from serhub_tasks where section_id = section_uuid and blocked = true) into has_blocked;
  if has_blocked then return 'blocked'; end if;
  select exists (select 1 from serhub_tasks where section_id = section_uuid and due_date < current_date and status < 100) into has_overdue;
  if has_overdue then return 'overdue'; end if;
  select exists (select 1 from serhub_tasks where section_id = section_uuid and due_date between current_date and current_date + interval '7 days' and status < 100) into has_approaching;
  if has_approaching then return 'approaching'; end if;
  select count(*), count(*) filter (where status = 100) into total_tasks, completed_tasks from serhub_tasks where section_id = section_uuid;
  if total_tasks > 0 and total_tasks = completed_tasks then return 'complete'; end if;
  return 'on_track';
end;
$$ language plpgsql stable;
...
returns text as $$
  select case when title is not null then title || ' ' || first_name || ' ' || last_name else first_name || ' ' || last_name end
  from serhub_profiles where id = user_uuid
$$ language sql stable;

create or replace function serhub_get_task_status_label(task_row serhub_tasks)
returns text as $$
begin
  if task_row.blocked then return 'blocked'; end if;
  if task_row.status = 100 then return 'complete'; end if;
  if task_row.due_date < current_date then return 'overdue'; end if;
  if task_row.due_date between current_date and current_date + interval '7 days' then return 'approaching'; end if;
  return 'on_track';
end;
$$ language plpgsql stable;
...
begin
  select coalesce(avg(serhub_get_section_progress(id)), 0) into total_progress from serhub_sections where level = 1;
  return round(total_progress);
end;
$$ language plpgsql stable;

create or replace function serhub_get_user_task_stats(user_uuid uuid)
returns table (owned_tasks integer, collaborating_tasks integer, reviewing_tasks integer, overdue_tasks integer, completed_tasks integer) as $$
begin
  return query select
...
create trigger serhub_profiles_updated_at before update on serhub_profiles for each row execute function serhub_update_updated_at();
create trigger serhub_sections_updated_at before update on serhub_sections for each row execute function serhub_update_updated_at();
create trigger serhub_tasks_updated_at before update on serhub_tasks for each row execute function serhub_update_updated_at();
create trigger serhub_meetings_updated_at before update on serhub_meetings for each row execute function serhub_update_updated_at();
create trigger serhub_notification_prefs_updated_at before update on serhub_notification_preferences for each row execute function serhub_update_updated_at();

create trigger serhub_tasks_audit after insert or update or delete on serhub_tasks for each row execute function serhub_log_audit();
create trigger serhub_profiles_audit after insert or update or delete on serhub_profiles for each row execute function serhub_log_audit();
create trigger serhub_meetings_audit after insert or update or delete on serhub_meetings for each row execute function serhub_log_audit();

create trigger on_auth_user_created after insert on auth.users for each row execute function serhub_handle_new_user();
